...
- 插件目录：`Plugins/Creta.Plugin.LocalPromptSearch`
- 动作关键字：`c` 保存，`v` 检索；`pt` 作为 `v` 的别名
- 目标：
  - 在本地维护 Prompt 模板库
  - 用 `c 文本` 把输入内容存成模板；只输入 `c` 时才回退到剪贴板。用 `v` 快速检索、复制并可选回焦粘贴
- 核心设计：
  - `c` 后面的文字就是 Prompt 正文，`#标签` 会单独抽出；标题取正文第一行
  - 未输入正文时，才读取剪贴板作为正文
  - `v` 的后续文本是搜索词；空查询展示最近使用、收藏和保存入口
  - 标题或正文重复时，优先提供更新已有模板，也可另存为新模板
  - `PromptRepository` 优先加载配置指定文件，否则加载插件目录下的 `prompts.json`，再回退到 `prompts.sample.json`
  - 若当前读的是示例文件，首次保存会改写到 `prompts.json`，避免污染 sample
